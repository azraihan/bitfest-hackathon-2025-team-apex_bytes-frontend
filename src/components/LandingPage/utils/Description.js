...
function Description({ descriptions }) {
  //descriptions: array of descriptions that contains the title, content, image, and a boolean value determining the order/ orientation of the text content and the image
  return (
    <>
      {descriptions.map((section, index) => (
        ...
        >
          {" "}
          {/**direction is used to tell in which order the items should be put*/}
          <Grid
            item //it means that this grid is an item
            xs={12}
            md={
              6
            } /* Content that will take full width (12 columns as a 12 column system has been adopted) on xs screens and half width (6 out of 12 columns) on md screens and up */
            sx={{
              display: "flex",
          ...
          >
            <Typography variant="h3" fontWeight="bold" className="hoverScale">
              {section.title}
            </Typography>
